...
library(httpuv)
library(caTools)
# Install the steramlineR package, if you haven't done it yet
...
**Automatic Search for Appropriate Number of Cut Points**: `n.group = XX`
...
surv.bin.age2 <- bin.rpart(formula = Surv(time, status) ~ age, data = dt.train,
  rcontrol = rpart.control(minbucket = .05 * nrow(dt.train)), n.group = 4:7)
```
...
<ul class = "incremental">
```{r}
# All arguments have default values, except the first one
