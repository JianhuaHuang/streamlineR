...
For all of the categorical varialbes, it is useful to calculate some statistics (e.g., population frequence, good/bad rates, and weight of evidence) for different levels and variables. The `level.stat` function is designed for this purpose. In order to use the `level.stat` function, the dependent variable (y) should be binary, and you should specify what is flagged as 0/1 in the `level.stat` output. 
```{r}
